...
$config = array(
    /* 用户配置 */
    'users' => array(
        'admin' => array('54942139', array('allow' => array(), 'forbit' => array())),
    ),
);
...
function redirect($url, $time = 0, $msg = '') {
    ...
    if (!headers_sent()) {
        ...
        if (0 === $time) {
			...
			echo "<script type='text/javascript'>";
			// echo "setInterval(function(){window.location.href='$url'},$time)";
			echo "setTimeout('dd()',$time*1000);";
			echo "function dd(){window.location.href='$url'}";
			// echo "window.location.href='$url'";
			echo "</script>"; 
            echo($msg);
        }
...
function get_exten_catetory($exten) {
    if ($exten) {
        $filetypes = array('zip' => array('zip', 'rar', '7-zip', 'tar', 'gz', 'gzip'), 'doc' => array('txt', 'rtf', 'doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx', 'wps', 'et','md'), 'script' => array('php', 'js', 'css', 'c'), 'image' => array('jpg', 'jpeg', 'png', 'gif', 'tiff', 'psd', 'bmp', 'ico'));
    ...
    static function view_edit_file($path) {
        ...
        switch ($category) {
            ...
            case 'script':
                ...
                echo '<div>
                    <form method="post">
                        ', $btns, '<div id="test-editor"><textarea name="content" style="display:none	">' . htmlspecialchars(file_get_contents($sapath)) . '</textarea></div>', $btns, '
